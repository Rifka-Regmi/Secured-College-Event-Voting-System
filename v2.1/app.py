        ...
        try:
            ...
            if len(faces):
                for (left, top, width, height) in faces:
                    ...
                    if detected_name != "Unknown" and confidence < FaceDetect.RECOGNITION_THRESHOLD:
                        try:
                            # Load profile data
                            ...
                            label = f"{detected_name} ({confidence:.1f})"
...
def otp():
    ...
    if request.method == "GET":
        if not otp_sent:  # Only generate and send OTP if not already sent
            otpValue = random.randint(1000, 9999)
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=80, debug=True)  #ocal run