...
# Move the training code into a function that's called only when needed
def initialize_face_recognition():
    try:
        cropped_face_location = "cropped_faces"
        cropped_label_location = "cropped_labels"
        face_recognizer = train(cropped_face_location, cropped_label_location)
        return face_recognizer
    except cv2.error:
        print("No training data available yet. Face recognition will be initialized later.")
        return None

# Global variable for face recognizer
face_recognizer = None

def recog(face_img, threshold=50):
    global face_recognizer
    if face_recognizer is None:
        face_recognizer = initialize_face_recognition()
        if face_recognizer is None:
            return "Unknown"
    
    # Rest of your recognition code
    # ...
